...
export default function NoteDetailPage() {
    ...
    async function callAI(type: "summary" | "improve" | "tags") {
        ...
        if (type === "improve") setImproved(data.improved);
        if (type === "tags") {
    ...
    const handleSummary = async () => {
        const result = await callAI("summary", note.content);
        setSummary(result);
    };
    const handleImprove = async () => {
        const result = await callAI("improve", note.content);
        setImproved(result);
    };
    const handleTags = async () => {
        const result = await callAI("tags", note.content);

        // clean tags (important!)
        const cleanTags = result
            .replace(/#/g, "")
            .split(",")
            .map(t => t.trim())
            .filter(Boolean);
